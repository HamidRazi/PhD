    ...
    plugin.trial = function(display_element, trial) {
        ...
        switch(trial.choice_type )
        {
            case "free":{
                ...
                display.left_info_image_number = trial.image_allocation[trial.left_info_image_number];
                display.right_info_image_number = trial.image_allocation[trial.right_info_image_number];
                display.left_bug_image_number = trial.image_allocation[trial.left_bug_image_number];
                display.right_bug_image_number = trial.image_allocation[trial.right_bug_image_number];
               
               
                if(trial.InfoAnswerNumber==0){
                    display.left_info_image_number +=7;
                    feedback_images
                    display.left_info_stimulus  = [trial.feedback_images[display.left_info_image_number]];
                }else
                {
                    display.left_info_stimulus  = [trial.info_stimuli[display.left_info_image_number]];
                }

                if(trial.InfoAnswerNumber==1){
                    display.right_info_image_number+=7;
                    display.right_info_stimulus = [trial.feedback_images[display.right_info_image_number]];
                }else{
                    display.right_info_stimulus = [trial.info_stimuli[display.right_info_image_number]];
                }
          
               
               
                display.left_bug_stimulus   = [trial.bug_stimuli[display.left_bug_image_number]];
                display.right_bug_stimulus  = [trial.bug_stimuli[display.right_bug_image_number]];
            ...
            case "forced":{
                ...
                if (Array.isArray(queues.null_trial_queue) && queues.null_trial_queue.length > 0) {
                    ...
                    display.left_info_image_number = trial.image_allocation[queues.null_trial_queue[0]]; // get the image number from the null trial queue
                    queues.null_trial_queue.shift(); // remove the first item of the null trial queue
                } else {
                    display.left_info_image_number = trial.image_allocation[jsPsych.randomization.sampleWithReplacement([...Array(queues.rinfosourceq.length).keys()], 1)];
                ...
                if (Array.isArray(queues.null_trial_queue) && queues.null_trial_queue.length > 0) {
                    ...
                    display.right_info_image_number = trial.image_allocation[queues.null_trial_queue[0]]; // get the image number from the null trial queue
                    queues.null_trial_queue.shift(); // remove the first item of the null trial queue
                } else {
                    display.right_info_image_number = trial.image_allocation[jsPsych.randomization.sampleWithReplacement([...Array(queues.rinfosourceq.length).keys()], 1)];
                }
                ...
                // check to see whether there is a null trial in the queue. If so, take it. If not, randomly select an image
                if (Array.isArray(queues.null_trial_queue) && queues.null_trial_queue.length > 0) {
                    display.left_bug_image_number = trial.image_allocation[queues.null_trial_queue[0]]; // get the image number from the null trial queue
                    queues.null_trial_queue.shift(); // remove the first item of the null trial queue
                } else {
                    display.left_bug_image_number = trial.image_allocation[jsPsych.randomization.sampleWithReplacement([...Array(queues.rbugmushq.length).keys()], 1)];
                }
                display.left_bug_stimulus  = [trial.bug_stimuli[display.left_bug_image_number]];
             
                // check to see whether there is a null trial in the queue. If so, take it. If not, randomly select an image
                if (Array.isArray(queues.null_trial_queue) && queues.null_trial_queue.length > 0) {
                    display.right_bug_image_number = trial.image_allocation[queues.null_trial_queue[0]]; // get the image number from the null trial queue
                    queues.null_trial_queue.shift(); // remove the first item of the null trial queue
                } else {
                    display.right_bug_image_number = trial.image_allocation[jsPsych.randomization.sampleWithReplacement([...Array(queues.rbugmushq.length).keys()], 1)];
                }
        ...
        function DrawScreen() {
            ...
            var b=-1;// 
            if(trial.QuestioniDsplayStatus==0){
                ctx.strokeRect(100, 40, 800, 230);// 
                b=-1}
            else    if(trial.QuestioniDsplayStatus==1) {b=1;
                ctx.strokeRect(100, 280, 800, 230);//
            }
          
            // if the left stimulus is selected, show the appropriate selection box
            if (display.left_box == "selected"){
                _DrawSelectionBox(-trial.stimulus_offset[0], trial.stimulus_offset[1]*b, trial.selection_colour);
            } else if (display.left_box == "Win"){
                ...
                _DrawSelectionBox(-trial.stimulus_offset[0], trial.stimulus_offset[1]*b, trial.reward_colour);
                ...
                display.right_bug_stimulus=null;                if(trial.InfoAnswerNumber==1)
                {
                    display.left_info_stimulus=null;
                ...
                _DrawSelectionBox(-trial.stimulus_offset[0], trial.stimulus_offset[1]*b, trial.non_reward_colour);
                _DrawFeedback(feedback_image);
            ...
            if (display.right_box == "selected"){
                _DrawSelectionBox(trial.stimulus_offset[0], trial.stimulus_offset[1]*b, trial.selection_colour);
            } else if (display.right_box == "Win"){
              
                display.right_bug_stimulus=[trial.feedback_images[5]];
                display.left_bug_stimulus=null;                if(trial.InfoAnswerNumber==1)
                {
                    display.left_info_stimulus=null;
                    display.right_info_stimulus=[trial.feedback_images[6]];
            ...
            if (display.left_bug_stimulus !== null){
                ...
                _DrawStimulus(display.left_bug_stimulus, [-trial.stimulus_offset[0], trial.stimulus_offset[1]*(b)]);
               
              
            } else{
                _DrawBlankStimulus([-trial.stimulus_offset[0], trial.stimulus_offset[1]*(b)]);
            ...
            if (display.right_bug_stimulus !== null){
                ...
                _DrawStimulus(display.right_bug_stimulus, [trial.stimulus_offset[0], trial.stimulus_offset[1]*(b)]);
            ...
            if (display.left_info_stimulus !== null){
                ...
                _DrawStimulus(display.left_info_stimulus, [-trial.stimulus_offset[0], trial.stimulus_offset[1]*(b*-1)]);
              
               
            } else{
                _DrawBlankStimulus([-trial.stimulus_offset[0], trial.stimulus_offset[1]*(b*-1)]);
            ...
            if (display.right_info_stimulus !== null){
                ...
                _DrawStimulus(display.right_info_stimulus, [trial.stimulus_offset[0], trial.stimulus_offset[1]*(b*-1)]);
        ...
        function DisplayFeedback(){
            ...
            } else if (trial.left_bug_image_type !== null){
            ...
            if (response.choice == "left"){
                ...
                if (trial.left_bug_image_type == "bugmush"){
              
                 
                    if (queues.otherq[left_image_number].length > 0){

                        response.feedback = queues.otherq[left_image_number][0];
                        queues.otherq[left_image_number].shift();
               
                    } else {
                    
                        response.feedback = jsPsych.randomization.sampleWithReplacement(queues.otherq[left_image_number],1)
                    }
                ...
                // get feedback from the queue and update the queue
                if (trial.right_bug_image_type == "bugmush"){
                    if (queues.otherq[right_image_number].length > 0){
                        response.feedback = queues.otherq[right_image_number][0];
                        queues.otherq[right_image_number].shift();
                    } else {
                        response.feedback = jsPsych.randomization.sampleWithReplacement(queues.otherq[right_image_number],1)
                    }
        ...
        function _DrawFeedback(feedback_image) {
            ...
            //if(trial.QuestioniDsplayStatus==0)
            //{
            //    if( trial.feedback_offset[1]>0){
            //        trial.feedback_offset[1]=-1*trial.feedback_offset[1];
            //    }
            //}else   if(trial.QuestioniDsplayStatus==1)
            //{
            //    if( trial.feedback_offset[1]<0){
            //        trial.feedback_offset[1]=1*trial.feedback_offset[1];
            //    }
